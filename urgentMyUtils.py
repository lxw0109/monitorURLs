...
def writeLog(tag, url, log):
    ...
    """
    with open("./urgentMonitorLog", "a") as f:
        logLock.acquire()
...
def writeFile(url, length, md5Str):
    ...
    """
    with open("./urgentCriterion_new", "a") as f:
        fileLock.acquire()
...
def getEmailContent(url, length, md5Str, checkTime, urlObjDic, sourceCode, aeURLs):
    ...
    """
    content = ""
    filename = "./urgentIntermedia/" + url.replace("/", "_")
    try:
...
def getEmailContent_stale(url, oldUrlObj, newUrlObj):
    ...
    content = ""
    filename = "./urgentIntermedia/" + url.replace("/", "_")
    filenameNew = "./urgentIntermedia_new/" + url.replace("/", "_")
...
def recordInFile(url, sourceCode):
    ...
    filename = "./urgentIntermedia_new/" + url
...
def cpFile(url):
    ...
    filename2 = "./urgentIntermedia_new/" + url
    try:
